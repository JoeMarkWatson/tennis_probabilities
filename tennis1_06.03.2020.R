...
P_Wtl <- d^4 #Player A wins game to love
P_Wt15 <- 4*(d^4)*m #Player A wins game to 15
#showing factorials working for one game win outcome only:
#for Player A to win to 15, Player A must win last point - d - and 3 of the previous 4 points - d^3(m)
#to est the number of possible ways in which 3 of the previous 4 points can be won, calculate C(n,r) where r = 4 (the number of previous points) and n =4 (the number of these points won by Player A)
#this shows there are 4 possible ways in which this outcome could be reached. So: 
#d*4d^3*m = 4*(d^4)*m
P_Wt30<-10*(d^4)*(m^2) #Player A wins game to 30
P_deuce<-20*d^3*m^3 #deuce reached
P_WfDeuce<-(d^2)/(1-(2*d*m)) #Player A wins game from deuce
P_DW<-P_Wtl+P_Wt15+P_Wt30+(P_deuce*P_WfDeuce) #probability of Player A winning game
P_MW<-1-P_DW #probability of Player B winning game
